...
- [x] UX critical fixes: 51 critical issues fixed across 52 files. Finance: dir="ltr" on ~40 currency amounts, paddingInlineStart, locale-aware fmt(), 2 pages fully translated. Bible/Songs: rtl:rotate-180 on all directional icons, touch targets h-6→h-10/h-7→h-10, presenter touch visibility. Auth/Dashboard/Permissions: ~50 hardcoded strings → t(), inline ternaries removed. Events/Members: touch targets min-h-[44px], 4 form wizard step titles translated, confirm()→AlertDialog. Prayer/Notifications/Visitors: touch targets h-7→h-9/h-10, filter tabs enlarged. 120+ translation keys added. 952 tests, 0 TS errors, 0 RTL violations.
...
|------|-----------|-------------|----------------|
| 2026-03-14 | UX critical fixes | 51 critical issues fixed: Finance dir="ltr" on ~40 currency amounts + paddingInlineStart + locale-aware fmt() + 2 pages fully translated + badge components use t(). Bible/Songs rtl:rotate-180 on all directional icons + touch targets h-6→h-10 + presenter touch visibility. Auth/Dashboard/Permissions ~50 hardcoded strings → t(). Events/Members touch targets min-h-[44px] + 4 form wizard step titles + confirm()→AlertDialog. Prayer/Notifications/Visitors touch targets enlarged. 120+ translation keys. 952 tests, 0 TS errors, 0 RTL violations. | 52 files: 12 finance pages, 7 bible/songs components, 7 auth/dashboard/permissions, 12 events/members/ministries, 6 prayer/notifications/visitors, 3 translation files |
