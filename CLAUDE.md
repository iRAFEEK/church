...
> Last updated: 2026-07-11 | Updated by: Staging environment (Phase 0) + full onboarding walkthrough — 3 bugs fixed (migrations 087–089 applied to STAGING; prod still pending 084–089)
...
| 083 | member_phone_admin_default.sql | **CEO decision (2026-06-25):** member phone numbers are ADMIN-ONLY by default. Changes `churches.member_directory_visibility` DEFAULT from `'leaders_only'` (set in 081) → `'hidden'`, and migrates existing churches still on the old default (`leaders_only` → `hidden`). Leaders regain phone access only when a super_admin grants the per-user `can_view_member_phone` permission. Pre-launch; no church had intentionally chosen `'leaders_only'`. |
...
| 087 | platform_admins.sql | **Ekklesia-admin approver allowlist (DB-managed).** `platform_admins (email PK, added_by, created_at)`, RLS **enabled with NO policies** = deny-all for anon/authenticated; all access via the service-role client in `app/api/platform/*`. Lets a platform operator add/remove approver emails from the UI at runtime instead of only via the `PLATFORM_ADMIN_EMAILS` env var. `isPlatformAdmin` (now async, `lib/platform.ts`) = email in env bootstrap (un-removable owner) **OR** row in this table. **NOT applied — human applies.** |
| 088 | member_join_pending.sql | **A-track: every member join needs church-admin approval.** Adds `'pending'` to the `user_churches.status` CHECK (active/managed/inactive/invited/**pending**) + rewrites `handle_new_user` so a **self-signup** membership is forced `status='pending'` (no app access until an admin approves). Ordering note: the `sync_user_churches_role` trigger (076), fired by the profiles INSERT inside `handle_new_user`, eagerly creates the row `'active'` — so 088 forces `'pending'` with an explicit trailing `UPDATE ... WHERE status='active'`. The two pre-approved doors set their own status AFTER the trigger: church founder → `register` upsert sets `'active'`; leader pre-add → `/api/members` sets `'managed'`/`'invited'`. **Auth-critical trigger change — NOT applied to prod; human applies via Supabase SQL editor.** APPLIED TO STAGING + behavior proven live (fresh signup → trigger produced `status='pending'`). |
| 089 | churches_anon_search.sql | **BUGFIX (caught on staging; live on prod too):** logged-out `/signup` church search was dead — middleware 307'd `/api/churches/search` to `/login` AND RLS only granted SELECT `TO authenticated` (031). Adds `"churches: anon search"` policy — anon SELECT on **ACTIVE churches only** (pending/rejected stay invisible). Paired code fix: middleware PUBLIC_PATHS entry + `requireAuth: false` on the search route. APPLIED TO STAGING; on prod an equivalent `"Anon can read active churches"` policy already existed (089 redundant there). |
| 090 | song_search_global.sql | **BUGFIX (user-reported "problem fetching songs"):** the whole 11,143-song hymnal is owned by ONE church (Eldobara) while songs SELECT RLS is app-wide — browse showed everything to every church but `search_songs_with_snippets` internally filtered `WHERE church_id = p_church_id OR church_id IS NULL`, so search returned NOTHING for every other church. Recreates the fn without the internal church filter (STABLE, runs under caller's RLS — visibility still governed by policies). Paired code fix in `/api/songs` GET: query sanitization (comma/paren no longer 500), caller church resolution, RPC-first (Arabic normalization + lyrics full-text + prefix) with ilike fallback, plain-text lyric snippets in results. **APPLIED TO STAGING + PROD 2026-07-11** (prod apply on explicit user instruction); verified cross-church on prod as a Grace member: "يا رب" → 3,222 hits, hamza-less "انت الاله" → 237 hits. |
| 091 | service_builder_segments.sql | **FEATURE (Service Builder):** `event_segments` gains `kind` ('generic'\|'song'\|'bible'\|'file' CHECK, default generic), `song_id` (FK songs, SET NULL), `bible_ref` JSONB ({bibleId, chapterId, reference, verse?}), `attachment_url`/`attachment_name`/`attachment_type`. New PUBLIC storage bucket `service-attachments` (25 MB; pdf/pptx/ppt/image mime-restricted; church-scoped unguessable paths via `/api/events/segments/upload` service-role route; public-read policy for projector display). Lets a run-of-show item BE a song (present via `/presenter/songs`), a Bible passage (`/presenter/bible`), or an uploaded slide deck/PDF. Paired: segments API GET/PUT/**POST-append** round-trip the fields; "Add to service" on Songs + Bible; the Add-Segment song/bible steps reuse the real `SongsTable` (pick mode) + Bible reader passage view. **APPLIED TO STAGING + PROD 2026-07-12** (prod apply on explicit user sign-off naming `hronbmjlklylupkbvgve`; verified all 6 cols + bucket + read policy + song index live). Code deployed to prod via `main` push (commits 0476a6c + 18a75ec). |
...
- [x] Domain health monitor — hourly cron detects the TLS-cert-binding outage class (HTTP up / HTTPS "no peer certificate available") + emails an alert — 2026-07-20
...
- [ ] Set `ALERT_EMAIL` (+ confirm `RESEND_API_KEY`) in Vercel Production so the hourly domain-health cron can actually email on an outage — without them it logs only. See OPERATIONS_RUNBOOK §7a.
...
- [x] Renumber duplicate migrations — **DONE** (the two `032_*` → `032b`, the two `033_*` → `033b`; see §5). Adopt Supabase timestamped filenames for *new* migrations going forward.
...
HEALTH_CHECK_URL=                  # Optional: URL the hourly domain-health cron probes (appends /api/health); falls back to NEXT_PUBLIC_APP_URL then https://www.miaekklesia.com
ALERT_EMAIL=                       # Optional: recipient for domain-health DOWN alerts; falls back to first PLATFORM_ADMIN_EMAILS entry
```
...
| 2026-07-20 | Doc-accuracy pass (8 fixes) + new gaps doc | **Verified-in-code documentation correction, no behavior change.** (1) **Test count** unified to **1,141 across 74 files** ("and growing — run `npx vitest run`") across PRD, SYSTEM_DESIGN, ENGINEERING_ONBOARDING, DELIVERED, BUILDING_WITH_AGENTS, ENVIRONMENTS, and all `.claude/agents/*.md` (was 1,079/1,100+/~1,120). Dated change-log rows stating "1079 tests" left as historical ledger. (2) **PRODUCT_SPEC §5** membership statuses corrected to the five `active | managed | invited | inactive | pending` (added `pending` from migration 088, with per-status descriptions). (3) **PRODUCT_SPEC §2** platform-admin corrected — `isPlatformAdmin` is **async**: env `PLATFORM_ADMIN_EMAILS` **OR** `platform_admins` row (migration 087), per `lib/platform.ts`. (4) **CLAUDE.md §5** added the missing **migration 083** row (`member_phone_admin_default.sql` — default directory visibility `leaders_only`→`hidden`). (5) **BACKLOG** `select('*')` items marked DONE (verified zero in prod code; only matches are an assertion-of-absence in `app/api/songs/__tests__/route.test.ts`). (6) **DEV_ENVIRONMENT + ENGINEERING_ONBOARDING** — loud warning that `npm run dev` runs against the **PROD DB via `.env.local`** (verified target = `hronbmjlklylupkbvgve`); always use `npm run dev:staging`; long-term fix = repoint `.env.local` at staging. (7) **`lib/features.ts`** finance-flag comment corrected (comment-only) — the gate uses the synchronous global `isFeatureEnabled`, so a per-church `church_features` row cannot make finance reachable today. (8) **CLAUDE.md §10** removed the stale "renumber duplicate migrations" pending item (already done per §5). **NEW** `docs/STATE_AND_GAPS.md` — honest current "what's missing / what needs to be done" (Do-these-first + grouped sections, each item What·Why·Size·Owner). 1,141 tests, 0 TS, 0 RTL, build clean. No code behavior changed; not committed/pushed. | docs/{PRODUCT_SPEC,EKKLESIA_PRD,SYSTEM_DESIGN,ENGINEERING_ONBOARDING,DELIVERED,BUILDING_WITH_AGENTS,ENVIRONMENTS,BACKLOG,DEV_ENVIRONMENT}.md, docs/STATE_AND_GAPS.md (NEW), .claude/agents/*.md, lib/features.ts (comment), CLAUDE.md |
| 2026-07-20 | Domain health-check cron (TLS-outage detection) | **NEW monitoring cron.** Prod went down when Vercel's edge lost the TLS cert binding for miaekklesia.com — HTTP still answered, every HTTPS request died at the handshake with "no peer certificate available", and nobody knew until a human noticed. `GET /api/cron/domain-health` (hourly, `0 * * * *`) closes that gap: follows the existing cron pattern (`verifyCronAuth` first, `logger`, `NextResponse.json`), probes `HEALTH_CHECK_URL` → `NEXT_PUBLIC_APP_URL` → `https://www.miaekklesia.com` + `/api/health` with `cache:'no-store'`, `redirect:'follow'` and a 10s `AbortController` timeout, plus a parallel informational probe of the bare apex. **A TLS failure makes `fetch` THROW, so the catch block is the primary signal, not an edge case** — DOWN = threw OR non-2xx. On DOWN: `logger.error` + alert email via the existing `emailProvider` (Resend) to `ALERT_EMAIL` (fallback: first `PLATFORM_ADMIN_EMAILS` entry) carrying URL/status/error/timestamp + the `vercel certs issue …` remediation hint. **Degrades gracefully** — unconfigured Resend or missing recipient logs an error (still surfaces in Vercel logs/Sentry) and returns `alerted:false` instead of crashing; a provider rejection is caught too. **Always returns HTTP 200** — `ok` carries the verdict, 200 means the cron ran (a non-200 would make Vercel retry and double-send alerts); documented in-file. **Touches no DB and no church data** — pure external HTTP check, so no Supabase/`church_id` by design; no i18n (operator-facing email/JSON, never rendered in-app). 11 new tests (auth 401 ×2, UP 200 + fetch-args assertion, DOWN-on-throw asserting the alert payload, PLATFORM_ADMIN_EMAILS fallback, 3 graceful-degradation paths, non-2xx 503/500). 1131 tests, 0 TS, 0 RTL, build clean. **Not deployed — no push, prod untouched.** | app/api/cron/domain-health/route.ts (NEW), app/api/cron/domain-health/__tests__/route.test.ts (NEW), vercel.json, .env.example, OPERATIONS_RUNBOOK.md (§7a + TLS incident runbook), CLAUDE.md |
| 2026-07-11 | Staging environment (Phase 0) + full onboarding walkthrough — 3 real bugs caught + fixed | **NEW: isolated staging Supabase** (`ekklesia-staging`, ref `emlmlcrwdbccskxigefd`, us-west-2; creds in gitignored `.env.staging`, never in the repo). Tooling: `scripts/apply-migrations-staging.mjs` (all migrations sorted, `--from` resume, seed-only SKIP set, guarded by `scripts/lib/db-guard.mjs` `assertNotProd()` pinning prod ref `hronbmjlklylupkbvgve`), `scripts/seed-staging.mjs`, `npm run dev:staging` (port 3100, staging env + `PLATFORM_ADMIN_EMAILS=platform@staging.test`), `.claude/launch.json` entry. All migrations 001–089 applied to staging (fixed drift in 022: `need_id`→`service_need_id`, phantom `notifications`-table indexes commented out). **Browser-verified end-to-end on staging:** Flow A (register → pending church + active founder → restricted shell/banner/trimmed nav/deep-link bounce → platform approve in `/platform` → full unlock), Flow B (self-signup → trigger-pending → `/membership-pending` → pastor approves in `/admin/join-requests` → tabs appear), Track 2 (Ekklesia Admin hub + Approvers panel, env owner locked), Flow C (login lands in last church; in-app join request → approve → switcher lists both → **switch works**). **3 real bugs found + fixed:** (1) logged-out `/signup` church search dead on prod — middleware redirect + authenticated-only RLS → migration 089 + PUBLIC_PATHS + `requireAuth:false` (smoke test updated); (2) join-requests queue 500 — PostgREST can't embed `profiles` on `user_churches.user_id` (FK is to auth.users) → two-step fetch in route + page; (3) **church switching 500 for every multi-church user since migration 050** — `prevent_self_role_escalation` trigger blocks self `church_id`/`role` update on the user-bound client → switch route now verifies ACTIVE membership (`isActiveMembership`) then updates via `createAdminClient()` (privilege-escalation tests updated + green). 1079 tests, 0 TS, 0 RTL, i18n 2742×3. **Prod promotion pending:** apply 084–089, deploy, set `PLATFORM_ADMIN_EMAILS` in Vercel. | .env.staging (NEW, gitignored), scripts/{apply-migrations-staging,seed-staging}.mjs (NEW), scripts/lib/db-guard.mjs (NEW), scripts/dev-staging.sh (NEW), package.json, .claude/launch.json, supabase/migrations/{089_churches_anon_search.sql (NEW),022_performance_indexes.sql}, middleware.ts, app/api/churches/{search,join-requests,switch}/route.ts, app/(app)/admin/join-requests/page.tsx, app/api/churches/switch/__tests__/privilege-escalation.test.ts, lib/api/__tests__/smoke-public.test.ts, docs/ENVIRONMENTS.md, CLAUDE.md |
| 2026-07-10 | Onboarding approval gates: pending-church, member approval, Ekklesia admin, multi-church UX | **4 tracks, code-complete + tested (1079 unit tests, 0 TS, 0 RTL, full i18n parity 2742×3, `npm run build` green). 2 new migrations NOT yet applied — human applies via Supabase SQL editor.** **Track 4 (every member join needs church-admin approval):** migration 088 adds `'pending'` to `user_churches.status` + rewrites `handle_new_user` so self-signup → pending (founder + leader-pre-add excepted, set active/managed after the trigger); `register` upsert now sets founder `status:'active'`; `/api/churches/join` first-join → pending; `/api/churches/join-requests` GET/PATCH extended to surface + approve pending same-church first-join memberships (union with cross-church `church_join_requests`); `lib/auth.ts` non-active membership now redirects to new `app/membership-pending/page.tsx` (not `/login`); `apiHandler` gains `requireActiveChurch` (applied to `/api/members` + `/api/notifications/send`). **Track 1 (pending church = info + tutorials only):** `(app)/layout.tsx` softened — pending church renders a restricted shell (allowlist `/dashboard`,`/profile`,`/help`,`/admin/settings` via new `x-pathname` middleware header) instead of a dead-end; nav trimmed via `getNavForUser(...,{isPendingChurch})`; `/dashboard` shows an "under review" home; banner in `AppShell`. **Track 2 (Ekklesia admin + email-managed approvers):** migration 087 `platform_admins`; `isPlatformAdmin` async (env OR table); `/api/platform/admins` GET/POST/DELETE (owner/last-admin guards); hub `app/(app)/platform/page.tsx` (pending-church queue + `ApproverManager`); nav entry via `{isPlatformAdmin}`. **Track 3 (multi-church):** login (+PhoneLoginForm) land in last church (dropped forced `/select-church`); `ChurchSwitcher` always-dropdown + "Join another church" + pending-request rows; new `app/(app)/churches/join` page (search active → request); `my-churches` filtered to active; new `/api/churches/my-requests`. New tests: async `isPlatformAdmin`, `requireActiveChurch`, pending-nav allowlist, first-join pending + approval flip. | supabase/migrations/{087_platform_admins,088_member_join_pending}.sql (NEW), app/(app)/layout.tsx, middleware.ts, lib/{platform,navigation,auth}.ts, lib/api/handler.ts, app/(app)/dashboard/page.tsx, components/layout/{AppShell,Sidebar,BottomNav,MoreSheet,ChurchSwitcher}.tsx, app/(app)/platform/{page,loading}.tsx (NEW), app/(app)/platform/churches/page.tsx, app/api/platform/{admins/route (NEW),churches/route}.ts, components/platform/ApproverManager.tsx (NEW), app/membership-pending/page.tsx (NEW), app/(app)/churches/join/{page,loading}.tsx (NEW), components/churches/{JoinAnotherChurch (NEW),JoinRequestList}.tsx, app/(app)/admin/join-requests/page.tsx, app/api/churches/{join,join-requests,my-churches,my-requests (NEW),register}/route.ts, app/(auth)/login/page.tsx, components/auth/PhoneLoginForm.tsx, app/api/members/route.ts, app/api/notifications/send/route.ts, messages/{en,ar,ar-eg}.json, lib/__tests__/{platform (NEW),navigation,auth}.test.ts, lib/api/__tests__/handler.test.ts, app/api/churches/__tests__/{join,join-requests,register}.route.test.ts, CLAUDE.md |
|------|-----------|-------------|----------------|
| 2026-07-10 | In-app visual guide (/help) + media pipeline | **NEW FEATURE:** low-literacy Arabic visual guide inside the app. `/help` library (role-filtered category cards, one open at a time) + `/help/[id]` lesson pages (step cards: icon + one spoken-Arabic sentence + real Majed voiceover + annotated screenshot + short MP4 screen recording). Server-side role gating (above-role deep links redirect). Discovery: 1 nav entry (Resources, all roles) + the existing Help FAB now links "Watch how this page works" → current page's lesson via `lessonIdForPath` (route→lesson map). Media: 398 real files in `public/help-guide` (130 jpg annotated in-page at capture, 78 **H.264 MP4** clips — WebM didn't play in Safari — 190 m4a voiceovers), captured from the running app in Arabic via the data-driven Playwright pipeline in `docs/visual-guide/` (manifest.mjs = single source of truth; capture → audio → gen-app-data regenerates everything incl. `lib/help/guide-data.ts`). helpGuide i18n ×3; nav test 31→32. Earlier same session: standalone RTL guide page (superseded), USER_GUIDE.md (59-section written guide), 4 prod-QA UI bugs fixed (raw i18n keys on event/notification types + t.has fallbacks, post-delete 404 → /events, double-asterisk labels). 1060 tests, 0 TS, 0 RTL. | app/(app)/help/** (NEW), components/help/{GuideLibrary,GuideLessonView}.tsx (NEW), components/help/TeachMeButton.tsx, lib/help/guide-data.ts (NEW, generated), lib/navigation.ts (+test), public/help-guide/** (NEW, 398 files), docs/visual-guide/** (pipeline), docs/USER_GUIDE.md, messages/{en,ar,ar-eg}.json, CLAUDE.md |
