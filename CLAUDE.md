...
### Pending / Not Started
- [x] **Migrations 087 + 088 APPLIED TO PROD (2026-07-11).** Verified live: `platform_admins` table created; `user_churches.status` CHECK now includes `pending`; `handle_new_user` rewritten (self-signup → pending); onboarding-approval feature fully live on miaekklesia.com. (084/085/086 were already applied earlier; 089 redundant on prod — it already had an `"Anon can read active churches"` policy.) Code deployed + `PLATFORM_ADMIN_EMAILS=ranytenma@gmail.com` set in Vercel (Production+Preview).
- [x] **Prod data hygiene DONE (2026-07-11):** the 30 `[SIM]` load-test churches were deactivated (`is_active=false, status='inactive`) in the same pass — public signup search now returns only the 7 real churches. (Rows retained/reversible, not hard-deleted.)
