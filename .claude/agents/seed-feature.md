...
- **Definition of Done for the seed file:** idempotent (safe to re-run) · every `_ar` column has real Arabic text · respects NOT NULL / CHECK / UNIQUE / FK constraints · scoped to the target church_id · `npx tsc --noEmit` stays at 0 if you touch any TS. The repo has 1,141 vitest tests across 74 files (and growing — run `npx vitest run` for the current number) — keep `npx vitest run` green if you change code.
