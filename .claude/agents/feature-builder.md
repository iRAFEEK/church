...
- **Tests:** the repo has 1,141 vitest tests across 74 files (and growing — run `npx vitest run` for the current number). Every new route/feature ships with tests (auth, church_id isolation, validation, role check) and keeps `npx vitest run` green.
