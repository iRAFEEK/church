...
export default async function PsalmodyDetailPage({ params, searchParams }: PageProps) {
  ...
  const section = sectionData as LiturgicalSection | null
  if (!section) notFound()
    ...
    <div className="space-y-6 pb-24">
      <div className="flex items-start gap-3">
        <Button variant="ghost" size="icon" className="size-11 shrink-0 -ms-2" asChild>
          <Link href="/liturgy/psalmody" aria-label={t('backToPsalmody')}>
        ...
        <div className="flex items-center justify-center gap-2 pt-4">
          {page > 1 && (
            <Button variant="outline" size="sm" className="h-11" asChild>
              <Link href={`/liturgy/psalmody/${slug}?page=${page - 1}`}>
