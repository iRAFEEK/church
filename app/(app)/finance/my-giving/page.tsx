...
export default async function MyGivingPage() {
  ...
  const [
    ...
    supabase
      .from('donations')
      .select('id, amount, base_amount, currency, donation_date, payment_method, receipt_number, is_tithe, fund:fund_id(id, name, name_ar), campaign:campaign_id(id, name, name_ar)')
  ...
  return (
    <div className="space-y-6 p-6 max-w-2xl mx-auto">
      ...
      {/* Active Pledges */}
      {(pledges as any[] || []).filter((p: any) => p.status === 'active').length > 0 && (
        <Card>
          ...
          <CardContent className="space-y-3">
            {(pledges as any[] || []).filter((p: any) => p.status === 'active').map((p: any) => {
              const pct = p.total_amount > 0 ? Math.min(100, (p.fulfilled_amount / p.total_amount) * 100) : 0
              return (
                <div key={p.id} className="space-y-1">
                  <div className="flex items-center justify-between text-sm">
                    <span>{p.campaign ? (isAr ? p.campaign.name_ar || p.campaign.name : p.campaign.name) : t('generalPledge')}</span>
