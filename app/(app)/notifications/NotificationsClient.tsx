...
interface NotificationsClientProps {
  /** Page-1 default-filter payload, server-fetched so 3G users see content with the page. */
  initialData: NotificationsInitialData | null
  /** Server-known role — members can never send, so they skip the scopes round trip. */
  initialRole: string
}

export function NotificationsClient({ initialData, initialRole }: NotificationsClientProps) {
  ...
  // Notification list state — seeded from the server payload (no blank wait on 3G)
  ...
  // Load user scopes on mount — skipped for plain members: getSendableScopes()
  // hard-returns canSend=false for role 'member', so the round trip is pure waste
  // for the vast majority of users on the busiest screen.
  useEffect(() => {
    if (initialRole === 'member') return
  ...
  // Skip the very first fetch when the server already provided page-1 default data;
  // any page/filter change re-creates fetchNotifications and fetches normally.
    ...
    <div className="space-y-6 pb-24">
      ...
      {totalPages > 1 && (
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center gap-2">
            <Button variant="outline" size="icon" className="h-9 w-9" disabled={page <= 1} onClick={() => setPage(p => p - 1)} aria-label={t('previousPage')}>
              <ChevronLeft className="h-4 w-4 rtl:rotate-180" />
            </Button>
            <Button variant="outline" size="icon" className="h-9 w-9" disabled={page >= totalPages} onClick={() => setPage(p => p + 1)} aria-label={t('nextPage')}>
