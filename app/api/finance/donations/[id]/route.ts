...
export const GET = apiHandler(async ({ supabase, profile, params }) => {
  ...
  if (error) {
    logger.error('[/api/finance/donations/[id] GET]', { module: 'finance', error })
...
export const PATCH = apiHandler(async ({ req, supabase, profile, params }) => {
  ...
  if (error) {
    logger.error('[/api/finance/donations/[id] PATCH]', { module: 'finance', error })
...
export const DELETE = apiHandler(async ({ supabase, profile, params }) => {
  ...
  if (error) {
    logger.error('[/api/finance/donations/[id] DELETE]', { module: 'finance', error })
    return NextResponse.json({ error: 'Internal server error' }, { status: 500 })
  }
  revalidateTag(`dashboard-${profile.church_id}`)
  revalidateTag(`funds-${profile.church_id}`)
