...
-- 1. Permissions JSONB on profiles
...
DROP POLICY IF EXISTS "role_permission_defaults_read" ON role_permission_defaults;
...
DROP POLICY IF EXISTS "role_permission_defaults_write" ON role_permission_defaults;
...
DROP POLICY IF EXISTS "permission_audit_log_read" ON permission_audit_log;
...
DROP POLICY IF EXISTS "serving_area_leaders_read" ON serving_area_leaders;
CREATE POLICY "serving_area_leaders_read"
  ON serving_area_leaders FOR SELECT
  TO authenticated
  USING (church_id IN (SELECT church_id FROM profiles WHERE id = auth.uid()));

DROP POLICY IF EXISTS "serving_area_leaders_write" ON serving_area_leaders;
...
DROP POLICY IF EXISTS "event_visibility_targets_read" ON event_visibility_targets;
...
DROP POLICY IF EXISTS "event_visibility_targets_write" ON event_visibility_targets;
...
-- ════════════════════════════════════════════════════════════
-- MIGRATION 025: Church-Wide Prayers & Outreach
-- ════════════════════════════════════════════════════════════
